...
g=fopen("nouv.txt","w");
	...
	{
		while (fscanf(f,"%s %s %s %s %s %s\n",rec2.ident,rec2.idNumBV,rec2.numlist,rec2.type,rec2.etat,rec2.MsgReclamation)!=EOF)
       { 
	if (strcmp(rec.ident,rec2.ident)!=0 || strcmp(rec.idNumBV,rec2.idNumBV)!=0 || strcmp(rec.numlist,rec2.numlist)!=0 || strcmp(rec.type,rec2.type)!=0 || strcmp(rec.etat,rec2.etat)!=0 || strcmp(rec.MsgReclamation,rec2.MsgReclamation)!=0)
{fprintf(g,"%s %s %s %s %s %s\n",rec2.ident,rec2.idNumBV,rec2.numlist,rec2.type,rec2.etat,rec2.MsgReclamation);		
	}
}
}
fclose(f);
...
/************************************************************/
/*void afficher_rec(GtkWidget *liste, char *filename)
{
reclamation rec;
...
if (store == NULL)
{
   ...
   {  
	f=fopen(filename,"a+");
   	  while(fscanf(f,"%s %s %s %s %s %s \n",rec.ident,rec.idNumBV,rec.numlist,rec.type,rec.etat,rec.MsgReclamation)!=EOF)
       {
        sprintf(Numbv,"%d",rec.idNumBV);
       gtk_list_store_append(store, &iter);
       gtk_list_store_set(store, &iter, IDENT, rec.ident, IDNUMBV, Numbv,NUMLIST, rec.numlist, TYPE, rec.type,  ETAT, rec.etat, MSG, rec.MsgReclamation,-1);
       }
       fclose(f);
